...
from constants import SERVER_PORT, GROUPMNGR_ADDR, GROUPMNGR_TCP_PORT, PEER_UDP_PORT, N, SERVER_ADDR, PEER_TCP_PORT
import threading
import random
import time
import pickle
		...
		while True:
			...
			if msg[0] == -2:
				for address_to_send in PEERS:
					...
					sendSocket.sendto(message_pack, (address_to_send, PEER_UDP_PORT))
					print(f'Sent message {-1} to {address_to_send}')
				...
				if int(message_number[-1]) == myself:
					...
					for address_to_send in PEERS:
						sendSocket.sendto(message_pack, (address_to_send, PEER_UDP_PORT))
						print(f'Sent message {message_number} to {address_to_send}')
...
def waitToStart():
    global nMsgs  # Add global declaration here
